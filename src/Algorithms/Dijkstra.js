...
function mapBoardToNodeArray(board) {
    ...
    board.forEach((e, index) => {
        unvisitedNode.push({
            index,
            row: index / constant.MAZE_RUNNER_ROW,
            col: index % constant.MAZE_RUNNER_COL,
            value: e,
            distance: Infinity,
            previousNode: null,
        })
...
function getUnvisitedNeighbor(node, grid) {
    ...
    if (row < constant.MAZE_RUNNER_ROW) neighbors.push(grid[(row + 1)*constant.MAZE_RUNNER_COL + col]);
    if (col > 0) neighbors.push(grid[row*constant.MAZE_RUNNER_COL + (col - 1)]);
    if (col < constant.MAZE_RUNNER_COL) neighbors.push(grid[row*constant.MAZE_RUNNER_COL + (col + 1)]);
